...
- Compute stats entirely client-side from the viewer's existing participation + concert data. No new Firestore reads beyond what the home view already does. No Cloud Function. No new data model.
...
- **Data:** none. Stats are derived live from existing `concerts` + `participations`. No new fields, no new collections, no new indexes.
- **Code:**
