# year-statistics Specification

## Purpose

Provide a per-year, per-user reflective statistics view that turns a friend group's accumulated attendance history into a small annual moment — summarising how many concerts the viewer attended, their best month, their top co-attendance buddies, their most-listened genre, their carpool balance, their venue diversity, and the year's first and last show. All values are computed client-side from the existing `concerts` and `participations` collections; there is no new stored data, no new backend, and (in v1) no charts — just big numbers and plain language on a mobile-first tile grid.
